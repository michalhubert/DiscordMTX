...
type AccessStatus = "checking" | "pending" | "approved" | "offline" | "denied" | "kicked";
...
export default function JoinGate({ path, whepUrl, isPrivate, bypass }: Props) {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (!cancelled) {
          if (data.status === "approved") {
            ...
            setStatus("kicked");
            setKickedUntil(typeof data.kickedUntil === "number" ? data.kickedUntil : Date.now());
    ...
    const interval = setInterval(poll, status === "kicked" || status === "denied" ? 1000 : 3000);
    return () => {
      cancelled = true;
  ...
  if (status === "kicked" && kickedUntil) {
    ...
    return (
      <div className="h-full w-full flex flex-col items-center justify-center gap-3 bg-black text-gray-300">
        ...
        <p className="text-lg font-medium">You have been kicked by the streamer</p>
        <p className="text-sm text-gray-500">
          You can rejoin the wait room in{" "}
          <span className="text-red-400 font-semibold">{formatTimeRemaining(timeRemaining)}</span>
        </p>
      </div>
    );
  }
