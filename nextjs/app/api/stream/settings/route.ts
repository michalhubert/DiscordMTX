...
const DEGRADATION_PREFERENCES = ["balanced", "maintain-framerate", "maintain-resolution"];
...
function sanitizeSettings(input: unknown): StreamSettings | null {
  ...
  const degradationPreference = DEGRADATION_PREFERENCES.includes(raw.degradationPreference as string)
  ...
  return {
    ...
    videoBitrateKbps: clamp(raw.videoBitrateKbps, DEFAULT_STREAM_SETTINGS.videoBitrateKbps, 100, 50000),
    videoCodec,
    contentHint,
    degradationPreference,
    includeAudio: raw.includeAudio !== undefined ? !!raw.includeAudio : DEFAULT_STREAM_SETTINGS.includeAudio,
    audioBitrateKbps: clamp(raw.audioBitrateKbps, DEFAULT_STREAM_SETTINGS.audioBitrateKbps, 6, 512),
  };
}
